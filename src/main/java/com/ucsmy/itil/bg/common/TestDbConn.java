...
public class TestDbConn {
    ...
    private static final int idletimeout = 2 * 60 * 1000;//一个连接idle状态的最大时长（毫秒），超时则被释放（retired），缺省:10分钟
    private static final int maxlifetime = 5 * 60 * 1000;//一个连接的生命时长（毫秒），超时而且没被使用则被释放（retired），缺省:30分钟，建议设置比数据库超时时长少30秒，参考MySQL wait_timeout参数（show variables like '%timeout%';）
    ...
    static {
        try {
            ...
            InputStream in = Object.class.getResourceAsStream("/mybatis/testcmdb.properties");
    ...
    public static void initConnectionPool() {
        ...
        config.setPoolName("testcmdb_153");
